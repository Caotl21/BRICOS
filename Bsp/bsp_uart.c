...
#define UART_RX_BUF_SIZE 128  // 单次 DMA 接收缓冲池大小
...
/* 实例化映射表 */
static uart_ctx_t s_uart_ctx[BSP_UART_MAX] = {
    [BSP_UART_IMU1] = { DMA2_Stream2, DMA_Channel_4, {0}, NULL },
    [BSP_UART_IMU2] = { DMA1_Stream5, DMA_Channel_4, {0}, NULL },
    [BSP_UART_OPI]  = { DMA1_Stream1, DMA_Channel_4, {0}, NULL },
    [BSP_UART_OTA]  = { NULL, 0, {0}, NULL }
};
...
void bsp_uart_isr_handler(bsp_uart_port_t port) {
    ...
    DMA_Stream_TypeDef* dma_rx = s_uart_ctx[port].dma_rx_stream;
    ...
    if (USART_GetITStatus(uart, USART_IT_RXNE) != RESET) 
    {
        ...
        // 直接通过回调扔给上层
        if (s_uart_ctx[port].rx_cb != NULL) {
            s_uart_ctx[port].rx_cb(&rx_byte, 1);
        }
    }
    ...
    if (USART_GetITStatus(uart, USART_IT_IDLE) != RESET) 
    {
        ...
        if (dma_rx != NULL) 
        {
            ...
            // 【注意】这里必须清除 DMA 的传输完成等标志位，防止重新开启后立刻进中断
            // (你之前的代码漏了这一步，LIFCR/HIFCR 的配置在此处应该用标准库函数替代，如:)
            // DMA_ClearFlag(dma_rx, DMA_FLAG_TCIFx | DMA_FLAG_HTIFx);

            // 3. 计算实际接收的长度 (此处 UART_RX_BUF_SIZE 需和初始化时的值对应)
            uint16_t rx_len = UART_RX_BUF_SIZE - dma_rx->NDTR;

            // 4. 调用回调交出数据
            if (s_uart_ctx[port].rx_cb != NULL && rx_len > 0) {
                s_uart_ctx[port].rx_cb(s_uart_ctx[port].rx_buf, rx_len);
            }
...
void bsp_uart_start_dma_rx_circular(bsp_uart_port_t port, uint8_t *buf_addr, uint16_t buf_size) {
    if (port >= BSP_UART_MAX || buf_addr == NULL || s_uart_ctx[port].dma_rx_stream == NULL) return;
    ...
    DMA_InitStructure.DMA_Channel = s_uart_ctx[port].dma_channel;
...
uint16_t bsp_uart_get_dma_rx_remaining(bsp_uart_port_t port) {
    if (port >= BSP_UART_MAX || s_uart_ctx[port].dma_rx_stream == NULL) return 0;
    
    // 直接调用标准库函数获取当前 DMA 数据计数器的值，这就是剩余未接收的数据长度
    return DMA_GetCurrDataCounter(s_uart_ctx[port].dma_rx_stream);
}
