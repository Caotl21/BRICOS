...
void USART2_DMA_Init(void)
{
    ...
    // ---------------------------------------------------------
    // 1. 配置 USART2_RX 的 DMA (DMA1 Stream5 Channel4) - JY901S
    // ---------------------------------------------------------
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA1, ENABLE); // 开启 DMA1 时钟

    DMA_DeInit(DMA1_Stream5);
    
    DMA_InitStructure.DMA_Channel = DMA_Channel_4;              // 通道 4
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)&USART2->DR; // 外设地址：串口数据寄存器
    DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)JY901BFifo.JY901BRxBuf;     // 内存地址：我们的缓冲区
    DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;     // 方向：外设 -> 内存
    DMA_InitStructure.DMA_BufferSize = JY901BFifoSize;            // 缓冲区大小
    DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable; // 外设地址不增
    DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;     // 内存地址自增
    DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte; // 字节传输
    DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
    ...
    DMA_Cmd(DMA1_Stream5, ENABLE); // 开启 DMA 流
}
...
void USART3_DMA_Init(void)
{
    ...
    // ---------------------------------------------------------------------
    // 1. 配置 USART3_RX 的 DMA (DMA1 Stream1 Channel4) - Orange Pi 5pro
    // ---------------------------------------------------------------------
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA1, ENABLE); // 开启 DMA1 时钟
    ...
    DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)Serial_RxPWM_Control;     // 内存地址：控制信号存储区
    DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;     // 方向：外设 -> 内存
    DMA_InitStructure.DMA_BufferSize = ControlSignal_RxBuf_Size;            // 缓冲区大小
    DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable; // 外设地址不增
    DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;     // 内存地址自增
    DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte; // 字节传输
    DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
