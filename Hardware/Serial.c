...
void Fast_Serial_SendSensorPacket(void)
{
    ...
    packet[index++] = (uint8_t)(Serial_SensorData.gyro_z1 >> 8);
    packet[index++] = (uint8_t)(Serial_SensorData.gyro_z1 & 0xFF);
    packet[index++] = (uint8_t)(Serial_SensorData.Wquat_1 >> 8);
    packet[index++] = (uint8_t)(Serial_SensorData.Wquat_1 & 0xFF);
    packet[index++] = (uint8_t)(Serial_SensorData.Xquat_1 >> 8);
    packet[index++] = (uint8_t)(Serial_SensorData.Xquat_1 & 0xFF);
    packet[index++] = (uint8_t)(Serial_SensorData.Yquat_1 >> 8);
    packet[index++] = (uint8_t)(Serial_SensorData.Yquat_1 & 0xFF);
	packet[index++] = (uint8_t)(Serial_SensorData.Zquat_1 >> 8);
    packet[index++] = (uint8_t)(Serial_SensorData.Zquat_1 & 0xFF);
...
void USART2_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)
    {
        ...
        {
        case 0:
            ota_state = (rx_byte == OTA_1) ? 1 : 0; // 等待包头1
            break;
        case 1:
            ota_state = (rx_byte == OTA_2) ? 2 : 0; // 等待包头2
            break;
        case 2:
            ota_state = (rx_byte == OTA_3) ? 3 : 0; // 等待包头2
            break;
        case 3:
            if(rx_byte == OTA_4)
            {
                BootFlag_RequestEnterBootloader();
                NVIC_SystemReset(); // 触发系统复位，进入 Bootloader
            }
			ota_state = 0;
			break;
        }
