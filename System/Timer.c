...
/**
  ...
  * 参    数：无
  * 返 回 值：无
  * 注意事项：此函数配置为外部时钟，定时器相当于计数器,周期TIM_Period=20000-1为20ms，
  */
void Ctl_WatchDog_Timer_Init(void)
{
	...
	TIM_TimeBaseInitStructure.TIM_Prescaler = 8400 - 1;				//预分频器，即PSC的值
	TIM_TimeBaseInitStructure.TIM_RepetitionCounter = 0;			//重复计数器，高级定时器才会用到
	TIM_TimeBaseInit(TIM5, &TIM_TimeBaseInitStructure);				//将结构体变量交给TIM_TimeBaseInit，配置TIM3的时基单元	
	/*中断输出配置*/
	TIM_ClearFlag(TIM5, TIM_FLAG_Update);						//清除定时器更新标志位
																
	TIM_ITConfig(TIM5, TIM_IT_Update, ENABLE);					//开启TIM5的更新中断
	...
	/*NVIC配置*/
	NVIC_InitTypeDef NVIC_InitStructure;						//定义结构体变量
	NVIC_InitStructure.NVIC_IRQChannel = TIM5_IRQn;				//选择配置NVIC的TIM3线
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;				//指定NVIC线路使能
